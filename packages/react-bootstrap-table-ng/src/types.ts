...
import {
    ...
    FILTERS_POSITION_BOTTOM,
    FILTERS_POSITION_INLINE,
    FILTERS_POSITION_TOP,
    ROW_SELECT_DISABLED,
    ROW_SELECT_MULTIPLE,
    ROW_SELECT_SINGLE,
} from "./const";
...
export interface SelectRowProps<T> {
    ...
    | undefined;
    style?:
    | ((row: T, rowIndex: number) => React.CSSProperties | undefined)
    | React.CSSProperties
    | undefined;
    classes?:
...
export interface BootstrapTableProps<T extends object = any, K = number> {
    /**
     * Tells react-bootstrap-table-ng which column is unique.
     */
    keyField: string;
    /**
    ...
    | undefined;
    rowStyle?:
    | ((row: T, rowIndex: number) => React.CSSProperties)
    | React.CSSProperties
    | undefined;
...
export type TableColumnFilterProps<FV = any, T extends object = any> = Partial<{
    ...
    /**
     * Register a listener which will be called when column filter being triggered. If you return an array value, react-bootstrap-table-ng will adopt this value as the final filtered result.
     */
