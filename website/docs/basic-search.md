...
`react-bootstrap-table-ng` support a table search function just like legacy search in `react-bootstrap-table`. However, new way will be more easier to custom.

**[Live Demo For Table Search](pathname:///react-bootstrap-table-ng/storybook/index.html?selectedKind=Table%20Search)**  
**[API & Props Definition](./search-props)**   
...
* Enable search via [`search`](./search-props) prop on `ToolkitProvider`.
